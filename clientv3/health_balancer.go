...
func (b *healthBalancer) hostPortError(hostPort string, err error) {
	if b.endpoint(hostPort) == "" {
		logger.Println("clientv3/balancer: %q is stale (skip marking as unhealthy on %q)", hostPort, err.Error())
		return
	}
	...
	logger.Println("clientv3/balancer: %q is marked unhealthy (%q)", hostPort, err.Error())
}

func (b *healthBalancer) removeUnhealthy(hostPort, msg string) {
	if b.endpoint(hostPort) == "" {
		logger.Println("clientv3/balancer: %q was not in unhealthy (%q)", hostPort, msg)
		return
	}
	...
	b.unhealthyMu.Unlock()
	logger.Println("clientv3/balancer: %q is removed from unhealthy (%q)", hostPort, msg)
}
...
func (b *healthBalancer) cleanupUnhealthy() {
	...
	for k, v := range b.unhealthyHostPorts {
		if time.Since(v) > b.healthCheckTimeout {
			...
			logger.Println("clientv3/balancer: removed %q from unhealthy after %v", k, b.healthCheckTimeout)
		}
	}
	b.unhealthyMu.Unlock()
}
	...
	for {
		...
		case <-time.After(b.healthCheckTimeout):
			...
			if pinned == "" || b.isUnhealthy(pinned) {
				logger.Println("do next connect, current=", pinned)
				select {
...
func (b *healthBalancer) Up(addr grpc.Address) func(error) {
	...
	if b.pinAddr != "" {
		logger.Println("clientv3/balancer: %q is up but not pinned (already pinned %q)", addr.Addr, b.pinAddr)
		return func(err error) {}
	}
	...
	b.pinAddr = addr.Addr
	logger.Println("clientv3/balancer: pin", addr.Addr, ", others=", b.addrs)
	...
	return func(err error) {
		...
		b.pinAddr = ""
		b.mu.Unlock()
		logger.Println("clientv3/balancer: unpin %q (%q)", addr.Addr, err.Error())
	}
}

func (b *healthBalancer) endpoints() []string {
	b.mu.RLock()
	defer b.mu.RUnlock()
	return b.eps
}
...
func (b *healthBalancer) mayPin(addr grpc.Address) bool {
	...
	b.unhealthyMu.RUnlock()
	b.mu.RLock()
	logger.Println("health addr", b.addrs)
	...
	if elapsed := time.Since(failedTime); elapsed < b.healthCheckTimeout {
		logger.Println("clientv3/balancer: %q is up but not pinned (failed %v ago, require minimum %v after failure)", addr.Addr, elapsed, b.healthCheckTimeout)
		return false
	}
...
func grpcHealthCheck(client *Client, ep string) (bool, error) {
	logger.Println("start to check healther, ip=", ep)
	conn, err := client.dial(ep)
	if err != nil {
		logger.Println("start to check healther has err, ip=", ep, err)
		return false, err
	}
	defer conn.Close()
	...
	cancel()
	if err != nil {
		logger.Println("start to check healther has err11, ip=", ep, err)
	...
	logger.Println("start to check healther succ, ip=", ep)
